...
def MeasureSim(seqnames,seqdict):

    sims = []

    for i in range(len(seqnames)-1):
        for j in range(i+1,len(seqnames)):
            sims.append(sum(seqdict[seqnames[i]][k]==seqdict[seqnames[j]][k]
                            for k in range(len(seqdict[seqnames[i]]))) / len(seqdict[seqnames[i]]))

    return int(round(np.mean(sims),3)*1000)
    ...
    for stofile in stofiles:
        ...
        for depth in (5,10,20):
            ...
            if depth > len(seqnames):
                break
            for tryN in range(1,101):
                sampled = random.sample(seqnames, depth)
                similarity = MeasureSim(sampled,seqdict)
                outstk = os.path.join("SeqSim","sto","{}_{}_{}.sto".format(fam,depth,similarity))
                outafa = os.path.join("SeqSim","afa","{}_{}_{}.afa".format(fam,depth,similarity))
